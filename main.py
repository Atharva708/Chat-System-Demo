...
async def get_homepage():
    # Read Teams UI HTML from file
    try:
        with open("teams_ui.html", "r") as f:
            html_content = f.read()
        return HTMLResponse(content=html_content)
    except FileNotFoundError:
        # Fallback to inline HTML if file not found
        return HTMLResponse(content="<html><body><h1>Teams UI file not found. Please ensure teams_ui.html exists.</h1></body></html>")

@app.get("/old", response_class=HTMLResponse)
async def get_old_homepage():
    html_content = """
<!DOCTYPE html>
<html lang="en">
<head>
...
<body class="bg-gray-100 h-screen overflow-hidden">
    ...
    <script>
        ...
        function setupWebSocketHandlers() {
            ...
            // Handle file/attachment selection - auto send immediately (no button needed)
imageInput.addEventListener('change', function (e) {
  ...
  if (!ws || ws.readyState !== WebSocket.OPEN) {
    alert('Not connected yet. Please enter your name/id first.');
    e.target.value = '';
    return;
  }

  if (!file.type.startsWith('image/')) {
    alert('Currently only image files are supported for OCR.');
    e.target.value = '';
    return;
  }
  ...
  reader.onload = function (event) {
    ...
    // ✅ THIS is what triggers OCR automatically on server
